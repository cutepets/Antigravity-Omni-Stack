...
const DAY_MAP: Record<string, string> = {
  MON: 'Th? 2', TUE: 'Th? 3', WED: 'Th? 4', THU: 'Th? 5', FRI: 'Th? 6', SAT: 'Th? 7', SUN: 'Ch? nh?t',
}
...
export function ProductDetailView({ productId }: { productId: string }) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Ðã xoá s?n ph?m')
      queryClient.invalidateQueries({ queryKey: ['products'] })
      router.push('/products')
    },
    onError: (err: any) => {
      toast.error(err?.response?.data?.message || 'Không th? xoá s?n ph?m')
    },
  })
  ...
  if (isLoading) {
    return <div className="p-6 text-foreground-muted flex items-center justify-center h-40">Ðang t?i chi ti?t...</div>
  }

  if (!product || !detailTree) {
    return <div className="p-6 text-error text-center">Không tìm th?y s?n ph?m</div>
  }

  const handleDelete = () => {
    if (window.confirm(`Xoá s?n ph?m "${product.name}"?`)) {
  ...
  return (
    <div className="flex flex-col gap-6 p-6 pb-20 max-w-7xl mx-auto w-full">
          ...
          {product.deletedAt ? (
            <>
              {canUpdateProduct ? (
                <button
                  onClick={() => {
                    if (window.confirm('Khôi ph?c s?n ph?m này?')) {
                      inventoryApi.restoreProduct(product.id).then(() => {
                        toast.success('Ðã khôi ph?c s?n ph?m')
                        queryClient.invalidateQueries({ queryKey: ['products'] })
                        queryClient.invalidateQueries({ queryKey: ['product-detail', product.id] })
                      }).catch(() => toast.error('L?i khi khôi ph?c'))
                    }
                  }}
                  ...
                  title="Khôi ph?c"
                >
                  <RefreshCw size={15} /> <span className="text-sm font-medium">Khôi ph?c</span>
                </button>
              ) : null}
            </>
          ) : (
            <>
              {canCreateProduct ? (
                <button
                  ...
                  title="T?o b?n sao s?n ph?m này"
                >
                  <Copy size={16} />
                </button>
              ) : null}
              {canUpdateProduct ? (
                <button
                  ...
                  className="h-9 px-4 flex items-center gap-2 rounded-lg border border-border bg-primary-500 text-white hover:bg-primary-600 transition-colors"
                >
                  <Pencil size={15} /> <span className="text-sm font-medium">S?a thông tin</span>
                </button>
              ) : null}
              {canDeleteProduct ? (
                <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* L?T C?T TRÁI - THÔNG TIN CHUNG (kho?ng 3/10) */}
        <div className="card p-6 border border-border rounded-2xl flex flex-col gap-8 bg-background-secondary/30 lg:col-span-3">
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {product.deletedAt ? (
                  <span className="badge text-[10px] px-2 py-0.5 rounded-full uppercase bg-red-500/10 text-red-500 border border-red-500/20">Ðã xóa</span>
                ) : product.isActive !== false ? (
                  <span className="badge badge-success text-[10px] px-2 py-0.5 rounded-full uppercase bg-success/10 text-success border border-success/20">Ðang bán</span>
                ) : (
                  <span className="badge text-[10px] px-2 py-0.5 rounded-full uppercase bg-gray-500/10 text-gray-400 border border-gray-500/20">Ngung bán</span>
                )}
              </div>
          ...
          <div className="flex flex-col gap-3 text-sm border-t border-border pt-6">
            <div className="flex items-center justify-between"><span className="text-[11px] font-bold uppercase tracking-wider text-foreground-muted shrink-0">Phân lo?i</span><span className="font-medium text-foreground truncate max-w-[65%] text-right" title={product.category || ''}>{product.category || '—'}</span></div>
            ...
            <div className="flex items-center justify-between"><span className="text-[11px] font-bold uppercase tracking-wider text-foreground-muted shrink-0">Ðon v?</span><span className="font-medium text-foreground truncate max-w-[65%] text-right" title={activeUnit}>{activeUnit}</span></div>
            ...
            <div className="flex items-center justify-between"><span className="text-[11px] font-bold uppercase tracking-wider text-foreground-muted shrink-0">Tr?ng lu?ng</span><span className="font-medium text-foreground text-right">{activeWeight ? Number(activeWeight).toLocaleString('vi-VN') : '—'}</span></div>
            <div className="flex items-center justify-between"><span className="text-[11px] font-bold uppercase tracking-wider text-foreground-muted shrink-0">Nhãn hi?u</span><span className="font-medium text-foreground truncate max-w-[65%] text-right" title={product.brand || ''}>{product.brand || '—'}</span></div>
            ...
            <div className="flex items-center justify-between"><span className="text-[11px] font-bold uppercase tracking-wider text-foreground-muted shrink-0">Ca ki?m kho</span><span className="font-medium text-foreground text-right">
        ...
        {/* C?T PH?I - PHIÊN B?N VÀ CHI TI?T (kho?ng 7/10) */}
        <div className="lg:col-span-9 flex flex-col gap-6">
          {/* PH?N PHIÊN B?N & QUY Ð?I M?I (TRÊN CÙNG BÊN PH?I) */}
          <div className="card p-0 overflow-hidden border border-border rounded-2xl flex flex-col">
            <div className="p-4 border-b border-border font-semibold flex items-center justify-between text-[13px] tracking-wide uppercase text-foreground-muted bg-background-secondary/50">
              <div className="flex items-center gap-2">
                <Layers size={16} className="text-primary-500" /> Phiên b?n & Quy d?i
              </div>
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col leading-tight overflow-hidden">
                              ...
                              <div className="text-[10px] text-foreground-muted mt-0.5 truncate">{item.sku || product.unit || 'Phiên b?n'}</div>
                            </div>
                          </div>
                          {priceBooks.length > 0 && (
                            <div className="flex items-center gap-4 shrink-0">
                                ...
                                >
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex flex-col leading-tight overflow-hidden">
                                      ...
                                      <div className="text-[10px] text-foreground-muted mt-0.5 truncate">{(child.sku ? child.sku + ' • ' : '') + (child.formula || 'Quy d?i')}</div>
                                    </div>
                                  </div>
                                  {priceBooks.length > 0 && (
                                    <div className="flex items-center gap-4 shrink-0">
                      ...
                      <div key={item.key} className="relative">
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col leading-tight overflow-hidden">
                              <div className={`text-sm font-medium truncate ${activeItem.key === item.key ? 'text-primary-500' : 'text-foreground'}`}>{item.name}</div>
                              <div className="text-[10px] text-foreground-muted mt-0.5 truncate">{item.formula || item.sku || 'Quy d?i'}</div>
                            </div>
                          </div>
                          {priceBooks.length > 0 && (
                            <div className="flex items-center gap-4 shrink-0">
          ...
          {/* PH?N CHI TI?T T?N KHO/L?CH S? (DU?I CÙNG BÊN PH?I) */}
                ...
                <table className="data-table">
                  <thead className="sticky top-0 z-10 bg-background">
                    <tr>
                      ...
                      <th className="py-3 px-4 text-[11px] text-right">T?N KHO</th>
                      <th className="py-3 px-4 text-[11px] text-right">CÓ TH? BÁN</th>
                      <th className="py-3 px-4 text-[11px] text-right">ÐANG GIAO D?CH</th>
                      <th className="py-3 px-4 text-[11px] text-right">ÐANG V?</th>
                    </tr>
                  </thead>
                  <tbody>
                    {activeStockRows.length > 0 ? (
                            ...
                            <td className="py-3 px-4 text-right text-sm">
                              {reservedStock > 0 ? (
                                <Link
                                  ...
                                  title="Xem don hàng dang x? lý có ch?a s?n ph?m này"
                                >
                                  {reservedStock}
                                </Link>
                              ) : (
                            ...
                            <td className="py-3 px-4 text-right text-sm">
                              {incoming > 0 ? (
                                <Link
                                  ...
                                  title="Xem phi?u nh?p dang ch? hàng v? có ch?a s?n ph?m này"
                                >
                                  {incoming}
                                </Link>
                              ) : (
                      ...
                      <tr>
                        <td colSpan={5} className="py-8 text-center text-sm text-foreground-muted">
                          Chua có d? li?u t?n kho cho m?c dang ch?n
                        </td>
                      </tr>
                    )}
                  </tbody>
...
function resolveTransactionLink(tx: { referenceId?: string | null; referenceType?: string | null }) {
  ...
  switch (tx.referenceType) {
    ...
    case 'SUPPLIER_RETURN':
      return `/inventory/receipts` // supplier return không có route riêng, d?n v? danh sách
    case 'ORDER':
...
function ProductHistoryTab({ productId }: { productId: string }) {
  ...
  const typeLabel = (type: string) => type === 'IN' ? 'Nh?p kho' : 'Xu?t kho'
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border/50 bg-background">
        <div className="flex items-center gap-2 text-sm text-foreground-muted">
          ...
          <span className="font-medium text-foreground">
            {txs.length > 0 ? `${txs.length} giao d?ch` : 'L?ch s? xu?t nh?p'}
          </span>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            <option value="">T?t c? chi nhánh</option>
            {branches.map((b: any) => (
      ...
      <div className="overflow-auto flex-1">
        {isLoading ? (
          <div className="p-10 text-center text-sm text-foreground-muted">Ðang t?i l?ch s?...</div>
        ) : txs.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3 text-foreground-muted">
            <History className="h-10 w-10 opacity-30" />
            <p className="text-sm">
              {selectedBranchId
                ? `Chua có giao d?ch nào t?i ${branches.find((b: any) => b.id === selectedBranchId)?.name ?? 'chi nhánh này'}`
                : 'Chua có l?ch s? giao d?ch'}
            </p>
            {selectedBranchId && (
              <button
          ...
          <table className="data-table w-full">
            <thead>
              <tr>
                <th className="py-3 px-4 text-[11px] whitespace-nowrap">TH?I GIAN</th>
                ...
                <th className="py-3 px-4 text-[11px] whitespace-nowrap">HÀNH Ð?NG</th>
                <th className="py-3 px-4 text-[11px] text-right whitespace-nowrap">SL THAY Ð?I</th>
                <th className="py-3 px-4 text-[11px] whitespace-nowrap">LO?I</th>
                <th className="py-3 px-4 text-[11px] whitespace-nowrap">MÃ CH?NG T?</th>
              </tr>
            </thead>
            <tbody>
              {txs.map((tx: any) => {

...
Use this workflow when work has already crossed the threshold where structured phase artifacts are useful. Do not force obviously Quick tasks through phase planning.
</routing_alignment>
...
AGENT_SKILLS_RESEARCHER=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-researcher 2>/dev/null)
...
5. Commit:
```bash
node ".agent/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_DESC=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
UI_GATE_CFG=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node ".agent/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)

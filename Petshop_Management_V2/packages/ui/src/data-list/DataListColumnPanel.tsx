...
export function DataListColumnPanel({
  ...
  return (
    <div className="w-[calc(100vw-24px)] max-w-[320px] rounded-xl border border-border bg-background-secondary p-3 shadow-xl">
      <div className="flex items-start justify-between gap-3 px-1">
        <div>
          <div className="text-sm font-semibold text-foreground">Tùy chỉnh hiển thị cột</div>
        </div>
        <button
          type="button"
          onClick={closePanel}
      ...
      <div className="custom-scrollbar mt-3 max-h-[360px] space-y-2 overflow-y-auto pr-1">
        {columnOrder.map((columnId) => {
          ...
          return (
            <div
              ...
              className={`flex min-h-12 items-center gap-3 rounded-lg border px-3 py-2.5 transition-colors ${draggingColumnId === columnId
            ...
            >
              ...
              {onToggleSort && (
                <button
                  ...
                  disabled={!isSortable}
                  className={`inline-flex h-8 items-center gap-1 rounded-lg border px-2 text-xs font-semibold transition-colors ${isSorted

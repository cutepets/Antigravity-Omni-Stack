  ...
  const demoTransactions = Array.from({ length: 15 }, (_, index) => {
    ...
    if (index % 5 === 0) {
      return {
        ...
        notes: `POS trace: HOTEL_STAY:demo-hotel-${index + 1} | HOTEL_CODE:HOTELDEMO${String(index + 1).padStart(3, '0')}`,
        tags: `${DEMO_TAG},POS_ORDER,HOTEL_STAY:demo-hotel-${index + 1},HOTEL_CODE:HOTELDEMO${String(index + 1).padStart(3, '0')}`,

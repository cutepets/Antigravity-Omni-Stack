...
export const stockCountApi = {
  // Sessions
  createSession: (data: any) => api.post(`${API_BASE}/sessions`, data),
  getSessions: (params: { branchId: string; weekNumber?: number; year?: number; page?: number; limit?: number }) =>
  ...
  completeShiftSession: (shiftId: string) =>
    api.post(`${API_BASE}/shifts/${shiftId}/complete`),

  // Counting items
  submitCountItem: (itemId: string, data: { countedQuantity: number; notes?: string }) =>

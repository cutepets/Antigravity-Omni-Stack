...
import { staffApi, type StaffExcelPreviewResult } from '@/lib/api/staff.api'
import { cn } from '@/lib/utils'

export function StaffImportExportDropdown({
  canImport,
  onImported,
}: {
  canImport: boolean
  ...
  const [preview, setPreview] = useState<StaffExcelPreviewResult | null>(null)

  const handleExport = async () => {
    setIsOpen(false)
    setIsExporting(true)
    try {
      await staffApi.exportExcel()
      toast.success('Đã tải file Excel nhân viên')
    } catch (error: any) {
  ...
  const handleTemplate = async () => {
    ...
    try {
      await staffApi.downloadTemplate()
      toast.success('Đã tải mẫu nhập nhân viên')
    } catch (error: any) {
  ...
  const handlePreviewFile = async (file: File) => {
    ...
    try {
      const result = await staffApi.previewImport(file)
      setPreview(result)
  ...
  const handleApply = async () => {
    ...
    setIsApplying(true)
    try {
      const result = await staffApi.applyImport(selectedFile)
      setPreview(result)
      if (result.summary.errorCount > 0) {
        toast.error(`File Excel có ${result.summary.errorCount} lỗi, chưa thể áp dụng`)
        return
      }
      toast.success('Đã áp dụng dữ liệu nhân viên từ Excel')
      setPreview(null)
      setSelectedFile(null)
      onImported()
    } catch (error: any) {
    ...
    <>
      <div className="relative">
        <button
          ...
          className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-border bg-background-base px-3 text-xs font-semibold text-foreground-muted transition-colors hover:bg-background-tertiary hover:text-foreground disabled:opacity-50"
        >
          {isBusy ? <RefreshCw size={13} className="animate-spin" /> : <FileSpreadsheet size={13} />}
          Xuất/Nhập
          <ChevronDown size={13} />
        </button>

        {isOpen ? (
          <div className="absolute right-0 top-10 z-30 w-52 overflow-hidden rounded-xl border border-border bg-background-base shadow-xl">
              ...
              {isPreviewing ? (
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
                    {[
                      ['Tạo mới', preview.summary.createCount],
                      ['Cập nhật', preview.summary.updateCount],
                      ['Bỏ qua', preview.summary.skippedCount],
                      ['Lỗi', preview.summary.errorCount],
                      ['Cảnh báo', preview.summary.warningCount],
                    ].map(([label, value]) => (
                  ...
                  <div className={cn(
                    ...
                    {preview.summary.errorCount > 0
                      ...
                      : 'File hợp lệ. Áp dụng sẽ thêm mới/cập nhật hồ sơ nhân viên.'}
                  </div>

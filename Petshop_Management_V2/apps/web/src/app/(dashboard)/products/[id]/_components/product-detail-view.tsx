...
function resolveTransactionLink(tx: {
  referenceId?: string | null
  referenceType?: string | null
  referenceLookupId?: string | null
}) {
  const lookupId = tx.referenceLookupId ?? tx.referenceId
...
function ProductHistoryTab({
  ...
  const txs = useMemo(() => {
    const allTransactions: any[] = data?.data ?? []

    return allTransactions.filter((tx: any) => {
      if (sourceVariantId === undefined) return true
      if (sourceVariantId === null) return !tx.sourceProductVariantId && !tx.productVariantId
  ...
  const itemLabel = itemSku ? `${itemName} (${itemSku})` : itemName
                    ...
                    <td className="py-3 px-4 whitespace-nowrap">
                      {referenceCode ? (
                        txLink ? (
                          ...
                          <span className="font-mono text-xs text-foreground-muted">{referenceCode}</span>
                        )
                      ) : (

            ...
            <table className="w-full text-xs text-right whitespace-nowrap">
              ...
              <tbody>
                <tr className="border-b border-border/50">
                  ...
                  <td className="px-2 py-2.5">{snapshot.totalStock ?? fallback}</td>
                  <td className="px-2 py-2.5 text-primary-500 font-bold">
                    {snapshot.totalAvailableStock ?? fallback}
                  </td>
                  <td className="pl-2 py-2.5 text-foreground-muted">
                    {snapshot.totalTradingStock ?? fallback}
                  </td>
                </tr>
                ...
                {branches.filter((b: any) => b.isActive !== false).map((b: any) => {
                  ...
                  const reserved = bs ? (bs.reservedStock ?? (bs as any).reserved ?? 0) : 0
                  const available =
                  ...
                  return (
                    <tr key={b.id} className="border-b border-border/30 border-dashed last:border-0">
                      ...
                      <td className="pl-2 py-2 text-foreground-muted">{fallback}</td>
                    </tr>
                  )
                })}
              </tbody>

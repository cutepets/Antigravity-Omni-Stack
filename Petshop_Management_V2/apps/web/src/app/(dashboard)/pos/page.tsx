        ...
        <div className="order-4 lg:col-start-2 lg:row-start-2 bg-white lg:border-l border-gray-200 z-20 flex-1 flex flex-col p-4 overflow-y-auto">
          ...
          <div className="mt-auto flex flex-col gap-4">
            ...
            <div className="flex justify-between items-center py-2">
              <span className="text-base font-bold text-gray-800">KHÁCH PHẢI TRẢ</span>
              <span className="text-[22px] font-bold text-red-600">{moneyRaw(cartTotal)}</span>
            </div>

            <div className="mt-2 rounded-2xl border border-gray-200 bg-gray-50/80 p-4">
              {isMultiPaymentSummary ? (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[15px] font-medium text-gray-600">Khách đưa</span>
                    {allowMultiPayment ? (
                  ...
                  <div className="space-y-2">
                    {tabPayments.map((payment, index) => {
                      ...
                      return (
                        <div key={`${payment.paymentAccountId ?? payment.method}-${index}`} className="flex items-center justify-between gap-3 rounded-xl bg-white px-3 py-2.5">
                  ...
                  <div className="space-y-1 border-t border-dashed border-gray-300 pt-3">
                    ...
                    <div className={`text-xs font-semibold ${multiPaymentTotal >= cartTotal ? 'text-emerald-600' : 'text-rose-500'}`}>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0 justify-start">
                      <span className="shrink-0 text-[15px] font-medium text-gray-600 cursor-pointer" onClick={() => document.getElementById('customer_money_input')?.focus()}>
                        Khách đưa (F8)
                      </span>
                      <div ref={paymentMenuRef} className="relative shrink-0">
                        <button
                          type="button"
                          onClick={() => setIsPaymentMenuOpen((current) => !current)}
                          className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-sm font-semibold shadow-sm ${
                            getPaymentMethodColorClasses(currentSinglePaymentMethod?.type ?? 'CASH', currentSinglePaymentMethod?.colorKey).chip
                          }`}
                        >
                          <span className="max-w-[132px] truncate">{currentSinglePaymentMethod?.name ?? 'Chọn phương thức'}</span>
                          <ChevronDown size={14} />
                        </button>

                        {isPaymentMenuOpen ? (
                          <div className="absolute left-0 bottom-full z-40 mb-2 w-72 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl">
                              ...
                              {visiblePaymentMethods.map((method) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    {currentSinglePaymentMethod?.id === method.id ? <span className="text-[11px] font-bold text-primary-600 shrink-0 ml-2">Đang chọn</span> : null}
                                  </button>
                                );
                              })}
                              {allowMultiPayment ? (
                    ...
                    <div className="flex shrink-0 text-right">
                      {currentSinglePaymentType === 'CASH' ? (
                        <input
                          ...
                          className="w-32 border-b border-gray-300 bg-transparent pb-0.5 text-right text-base font-semibold outline-none focus:border-primary-500"
...
minimumMethods = { 2}
title = "Thanh toan nhieu hinh thuc"
description = "Chia tien thanh toan theo tung phuong thuc. Sau khi luu, panel POS se hien thi tung dong rieng de theo doi."

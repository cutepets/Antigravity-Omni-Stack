  ...
  return (
    <div className="flex animate-in fade-in slide-in-from-bottom-1 duration-200 flex-col gap-4 p-4 text-foreground">
      ...
      <div className="flex flex-col gap-2">
        {customizableShortcuts.map(([label, key]) => (
...
function PrintSettingsTab() {
  const { printerIp, setPrinterIp, paperSize, setPaperSize, autoPrint, setAutoPrint, autoPrintQR, setAutoPrintQR } = usePosStore()
  ...
  return (
    <div className="flex animate-in fade-in slide-in-from-bottom-1 duration-200 flex-col gap-4 p-4 text-foreground">
      ...
      <SettingsToggle label="Tự động in sau khi thanh toán" checked={autoPrint} onChange={setAutoPrint} />
      <SettingsToggle label="Tự động in mã QR khi chuyển khoản" checked={autoPrintQR} onChange={setAutoPrintQR} />
    </div>
  )
}
...
function PosSettingsTab({
  ...
  resolvedDefaultPaymentId,
  isPaymentMethodsLoading,
  isPaymentMethodsError,
}: {
  selectablePaymentMethods: any[]
  resolvedDefaultPaymentId: string
  ...
  return (
    <div className="flex animate-in fade-in slide-in-from-bottom-1 duration-200 flex-col gap-4 p-4 text-foreground">
        ...
        <div className="mt-1 grid grid-cols-3 gap-2">
          {THEMES.map(({ value, label, Icon }) => (
            <button
              ...
              className={`flex flex-col items-center gap-1.5 rounded-lg border px-2 py-2.5 text-xs font-semibold transition-colors ${posTheme === value
      ...
      <SettingsToggle label="Tự động focus ô tìm kiếm" checked={autoFocusSearch} onChange={setAutoFocusSearch} />
      <SettingsToggle label="Chế độ quét mã vạch" checked={barcodeMode} onChange={setBarcodeMode} />
      <SettingsToggle label="Âm thanh thao tác" checked={soundEnabled} onChange={setSoundEnabled} />
      <SettingsToggle label="Làm tròn tổng tiền" checked={roundingEnabled} onChange={setRoundingEnabled} />
        ...
        <div className="grid grid-cols-2 gap-2">
          {([100, 1000] as const).map((unit) => (
            <button
              ...
              className={`rounded-lg border px-3 py-2.5 text-sm font-semibold transition-colors ${roundingUnit === unit
                ? 'border-primary-500 bg-primary-500 text-white shadow'
                : 'border-border bg-surface text-foreground hover:border-primary-300'
                }`}
            >
      ...
      <div className="mt-2 flex flex-col gap-3 rounded-lg border border-border bg-surface-secondary p-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <input
            type="range" min="75" max="150" step="5" value={zoomLevel}
                ...
                return (
                  ...
                  >
                    <div className="flex w-full items-center justify-between gap-2">
                      ...
                      <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${isSelected ? 'bg-white/20 text-current' : colorClasses.chipSubtle}`}>
                        {(PAYMENT_METHOD_TYPE_LABELS as Record<string, string>)[method.type]}
                      </span>
                    </div>
...
// ── PosSettingsPanel: main component ─────────────────────────────────────────

export function PosSettingsPanel() {
  const [isOpen, setIsOpen] = useState(false)
  const [activeTab, setActiveTab] = useState<Tab>('POS')
  const activeBranchId = useAuthStore((state) => state.activeBranchId)
  const { zoomLevel, defaultPayment, setDefaultPayment } = usePosStore()
  ...
  const resolvedDefaultPaymentId = useMemo(() => {
    if (selectablePaymentMethods.some((method) => method.id === defaultPayment)) return defaultPayment
            ...
            <div className="no-scrollbar flex-1 overflow-y-auto">
              {activeTab === 'POS' && (
                <PosSettingsTab
                  ...
                  resolvedDefaultPaymentId={resolvedDefaultPaymentId}

...
import type { CartDiscountEditorProps } from './cart.types'

/**
 * CartDiscountEditor — Shared discount popover cho POS và Orders.
 * Không phụ thuộc POS store, dùng callback `onUpdateDiscountItem`.
 */
export function CartDiscountEditor({
    ...
    mobile = false,
}: CartDiscountEditorProps) {
    const updateDiscount = (value: number) => onUpdateDiscountItem(item.id, value)
    ...
    return (
        <>
            ...
            {mobile ? (
                <>
                    <div
                        className="text-[15px] font-medium text-gray-800 cursor-pointer border-b border-dashed border-gray-300 hover:border-primary-500 group-hover:border-primary-500 transition-colors w-fit pb-0.5"
                        onClick={onOpen}
                    >
